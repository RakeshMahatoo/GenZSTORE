...
const NewArrivals = () => {
 ...
 const handleMouseDown = (e) =>{
  ...
  const updateScrollButtons = () => {
    ...
    // check congtainer is lodded in dom
    if (container) {
  ...
  return (
    <section className="py-16 px-4 lg:px-0">
      <div className="container mx-auto text-center mb-10 relative">
        ...
        {/* scroll button */}
        <div className="absolute right-0 bottom-[-30px] flex sm:bottom-[-20px]  space-x-2">
          <button
            onClick={() => scroll("left")}
            disabled={!canScrollLeft}
            className={`p-2 rounded border ${canScrollLeft ? "bg-white text-black" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
          >
            <FiChevronLeft className="text-2xl border-gray-200 " />
          </button>
          <button onClick={()=> scroll("right")} disabled={!canScrollRight} className={`p-2 rounded border ${canScrollRight ? "bg-white text-black" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>
            <FiChevronRight className="text-2xl border-gray-200" />
          </button>
        </div>
      </div>
      {/* Scrollabel content */}
